...
| | |
|---|---|
| Версия | `0.6.6` (этап 6 принят архитектором на билде с кубиком; **пакет полировки окна**: прозрачность в билде, `windowToolWindow`, Ctrl+Alt+Q, оверлей без дрожания, мета VRM по спеке). Предыдущая серия — `0.6.5` (дедлок редактора), в переписке звалась `0.6.4.1` |
| Этап | 6 из 9 (`face-unity`) — ждём сборки Unity у Кирюши и команду «дальше» для этапа 7 |
| Python | 3.11+ |
| Тесты | **937 passed, 2 skipped** (pytest), 0 warnings · с tree-sitter — 939 |
...
| `VrmLoader.cs` | горячий своп тела (`Vrm10.LoadBytesAsync`): с диска или по HTTP. **0.6.5:** ждёт задачу по кадру (не `.Result`), с таймаутом и фазовыми логами |
...
**С 0.6.4 проба по умолчанию проверяет главное — «тело доезжает»** (хотфикс
блокера F7): берёт `hello.persona`, шлёт `persona_request`, сверяет `vrm` из кадра
с URL, который клиент строит сам, и **реально качает** модель по HTTP
(`200` + magic `glTF`). Именно этой проверки не хватало в 0.6.3: проба гоняла
аудио-тракт, а путь «подключился → тело» не проверялся никто, поэтому блокер
дожил до приёмки на Windows. Отключить — `--skip-body-check`.

```bat
python scripts\unity_face_probe.py
:: тело: lilith · GET 200 · 21192 Б · magic glTF · тело доедет: URL живой, GLB валиден
:: ИТОГ: 13 пройдено, 0 провалено
```

Если тела на диске нет, проба **краснеет** и печатает ответ сервера
(`404 {"status":"missing_vrm","detail":"модель не найдена: положи VRM по пути face.yaml:vrm_path"}`)
— то есть ровно ту причину, которую в 0.6.3 не было видно ни в Unity, ни в логе.
...
| **Unity: окно открыто, оверлей показывает персону, а в Hierarchy у `LilithFace` нет детей (тела нет), Console чистая** | Это блокер 0.6.3, закрыт в **0.6.4**. Причина: кадр `persona` сервер шлёт только в ответ на `persona_request`, а клиент его не просил — `VrmLoader.Swap()` не вызывался вовсе. Теперь клиент просит кадр сам, держит watchdog на 2 с и строит URL тела сам (`{ServerBaseUrl}/api/face/personas/{id}/model.vrm`), а старт/финиш/отказ свопа пишутся в Console безусловно. Обновить `Scripts/*.cs` и проверить лог `[Lilith] тело: старт свопа …` |
| **Unity: редактор виснет намертво при входе в Play (только через диспетчер задач), оверлей на «тело грузится…», в Console последняя строка «старт свопа»** | Это блокер 0.6.4, закрыт в **0.6.5** (в переписке 0.6.4.1). Причина: `yield return task` в Unity 6000.0 не ждёт завершения `Task`, поэтому код брал `.Result` у незавершённой задачи — `.Result` блокирует главный поток, а продолжения UniVRM планируются тем же потоком (`NextFrameTaskScheduler` → `UnityLoopTaskScheduler.Update`) → дедлок навсегда. Теперь ждём по кадру (`while (!loadTask.IsCompleted) yield return null;`), есть `Load Timeout Seconds` (60 с, 0 = вечно) и фазовые логи 0…7. Если загрузка всё же не завершается — в Console красный `ОТКАЗ … не завершилась за N с`, а не мёртвый редактор |
| Unity: тело грузится, но ругается на скин/кости | До 0.6.5 процедурный `tests/samples/test_cube.vrm` был битым: `JOINTS_0` объявлен `VEC4`, а записан одним `uint16` на вершину (accessor выходил за bufferView). Модель пересобрана, `validate()` проверяет геометрию accessor/bufferView. В 0.6.6 пересобрана ещё раз: мета приведена к спецификации VRM 1.0 (`commercialUsage: personalNonProfit` — значения `personal` в спеке нет), добавлены `creditNotation`/`modification`/`allowRedistribution`, размер **21 572 Б**. Пересобрать свою: `python scripts\make_test_vrm.py` |
| **Билд: окно есть, тело есть, а фон чёрный непрозрачный (режим `Dwm`)** | Замер Г: `DwmExtendFrameIntoClientArea` отрабатывает (в Player.log `прозрачность окна: Dwm (hwnd=…)`), но Unity 6 по умолчанию отдаёт кадр через **flip model swapchain** — альфа до DWM не доезжает. Лечение в **Player Settings**, не в коде: Resolution and Presentation → Fullscreen Mode = `Fullscreen Window`; Other Settings → **Use Flip Model Swapchain = ❌**. Плюс Quality → Anti Aliasing = `Disabled` (код и сам ставит `antiAliasing = 0`). Проверить режимы без ребилда: **F7** в окне (каждое переключение пишется в Player.log). Таблица — `unity-client/Assets/LilithFace/SCENE.md` §4.5 |
| Билд: окно «металось и рябило» в режиме `Layered Color Key` | До 0.6.6 стили и позиция окна применялись повторно при каждом кадре `persona`, а `SetLayeredWindowAttributes` вызывался вместе с `SetWindowPos`. Теперь Win32-вызовы происходят **только при смене режима**, а `SetWindowPos` — только если целевой прямоугольник действительно отличается от текущего (`Reposition(force)`). Плюс `QualitySettings.antiAliasing = 0` убирает кайму на цветовом ключе |
| Билд: окно не закрыть (рамок нет, в Alt+Tab не видно) | Так и задумано (`windowToolWindow = ✅` — оверлей не мешает переключению окон). Закрытие: **Ctrl+Alt+Q** (`Close Hotkey Enabled` / `Close Hotkey` в конфиге). Хочется крестик и рамку: `Show Window Frame = ✅` (прозрачность при этом не работает) или `Window Tool Window = ❌` — окно появится в Alt+Tab и таскбаре |
| Билд: строка оверлея `визема Rest · 0.00 · local` быстро мигает | `OnGUI` вызывается несколько раз за кадр (Layout/Repaint), а значения между вызовами меняются — глаз видит дрожание. В 0.6.6 текст оверлея собирается один раз в `Update` и рисуется только на событии `Repaint` |
| Билд: сначала появляется окно-сплэш Unity, потом наш квадрат | **Норма, не баг.** Сплэш — лицензионное окно плеера, оно создаётся до первой сцены и закрывается само. Отключается в Player Settings → Splash Image → Show Splash Screen ❌, но только на Unity Plus/Pro; на Personal его требует лицензия |
| **Unity: редактор мёртв при входе в Play, в Console одна-две строки «рукопожатие: …» и тишина** | Замеры Б/В хотфикса 0.6.5: точка зависания плавала внутри рукопожатия. С печати второй в клиенте нет ни блокирующих вызовов (`.Wait`/`.Result`/`Thread.Sleep`), ни кросс-поточных `lock` — только `ConcurrentQueue` и `socket.Abort()`. Для диагноза есть фазовые логи `h0…h9` с тегом потока и **канарейка** (раз в 0.5 с «главный поток ЖИВ» + счётчики очередей/кадров): канарейка молчит → поток заблокирован, последняя `hN` называет место; канарейка идёт, а кадры не растут → встал транспорт. Присылайте Console целиком |
| В логе сервера не видно, что Unity качал модель | Так и было до 0.6.4: uvicorn поднят с `access_log=False`. Теперь работает точечный access-лог `logging.access_log_prefixes` (дефолт `["/api/face/"]`): `HTTP GET /api/face/personas/lilith/model.vrm → 200 (21192 Б, 8.4 мс)`. Если строки нет — клиент не стучался |
| `[Lilith] тело: ОТКАЗ … 404 {"status":"missing_vrm"}` | У персоны нет тела: положи VRM по пути `personas/<id>/face.yaml: vrm_path` (или рядом как `model.vrm`). Проверить без Unity: `python scripts\unity_face_probe.py` |
