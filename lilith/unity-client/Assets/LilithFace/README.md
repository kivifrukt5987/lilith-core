...
|---|---|
| **Unity-окно подключается к WS** | оверлей: `Open · сервер 0.6.0 · персона lilith`; в логе сервера `Продюсер[unity]: подключён` |
...
| **Своп персоны меняет vrm + голос + карточку** | в веб-панели выбрать другую персону в селекторе 🎭 → Unity грузит новое тело, в логе `персона: nova` |
| **OBS берёт окно с прозрачностью** | в OBS: *Window Capture* → окно `LilithFace` → на десктопе и в OBS фон прозрачный |
